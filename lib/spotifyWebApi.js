  ...
  for (let i = 0; i < missingIds.length; i += TRACKS_BATCH_MAX) {
    ...
    if (!response.ok) {
      ...
      return {
        ok: false,
        status: response.status,
        code: "SPOTIFY_TRACKS_BATCH_FAILED",
        details
      };
    }
    const payload = await response.json();
 ...
 * Paginates over Spotify's combined library until enough owned rows are collected.
 */
async function spotifyListCurrentUserPlaylists({ sessions, persist, limit = 50, offset = 0 }) {
  ...
  while (owned.length < targetCount && spotifyHasMore && pagesFetched < MAX_SPOTIFY_PLAYLIST_PAGES) {
    ...
    for (const raw of page.rawItems) {
      ...
      if (isPlaylistOwnedByUser(summary, profile)) {
        owned.push(summary);
      }
    }
  ...
  const items = owned.slice(safeOffset, safeOffset + safeLimit);
  await enrichPlaylistTrackCounts({ sessions, persist, summaries: items });
  const hasMoreOwned = owned.length > safeOffset + safeLimit;
  const nextOffset = hasMoreOwned || spotifyHasMore ? safeOffset + items.length : null;
  return { ok: true, items: items.map(stripPlaylistSummaryInternals), nextOffset };
}
...
async function spotifyFetchLikedSongsSummary({ sessions, persist }) {
  const page = await spotifyListSavedTracks({ sessions, persist, limit: 1, offset: 0 });
  if (!page.ok) {
    return page;
  }
  const trackCount =
    typeof page.collectionTotal === "number" ? page.collectionTotal : (page.results || []).length;
  return {
    ok: true,
    likedSongs: {
