...
function normalizePlaylistTrackRow(row) {
  ...
  const normalized = normalizeTrack(track);
  if (!normalized.permalinkUrl) return null;
  // Bare playlist tracks only have upload created_at — not "added to playlist".
  if (isWrapped) {
    const addedAt = parseAddedAt(row.created_at);
    if (addedAt) normalized.addedAt = addedAt;
  }
...
async function soundCloudListLibrary({
  ...
  const likesResult = fetchSongCounts
    ? await soundCloudFetchLikesSummary({ accessToken })
    : { ok: true, likes: soundCloudLikesEntryWithoutCount() };
  if (!likesResult.ok) {
    return likesResult;
  }
