...
/**
 * Remove all up-next rows; keep the now-playing track when currentIndex >= 0.
 * When idle, clears the entire queue.
 * @param {unknown[]} queue
 * @param {number} currentIndex - -1 when nothing is playing
 */
function clearUpcomingQueue(queue, currentIndex) {
  const list = Array.isArray(queue) ? queue : [];
  if (currentIndex < 0) {
    return {
      ok: true,
  ...
  if (currentIndex >= list.length) {
    return { ok: false, error: "invalid current index" };
  }
  const playing = list[currentIndex];
  return {
    ok: true,
    nextQueue: [{ ...playing, status: "playing" }],
    nextCurrentIndex: 0,
    queueEmpty: false
  };
}
