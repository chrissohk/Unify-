...
function extractPlaylistTrackOrder(playlistPayload) {
  if (Array.isArray(playlistPayload?.track_order) && playlistPayload.track_order.length) {
    return playlistPayload.track_order.map((id) => String(id));
  }
  if (Array.isArray(playlistPayload?.tracks)) {
    return playlistPayload.tracks
...
function orderTracksByIdList(tracks, idOrder) {
  if (!idOrder?.length) return tracks;
  const map = new Map(tracks.map((t) => [String(t.id), t]));
  const ordered = idOrder.map((id) => map.get(String(id))).filter(Boolean);
  const seen = new Set(ordered.map((t) => String(t.id)));
  for (const t of tracks) {
    if (!seen.has(String(t.id))) ordered.push(t);
  }
  return ordered;
}

async function fetchSoundCloudPlaylistTrackOrder({ accessToken, playlistId, secretToken }) {
  const query = { representation: "full" };
  if (secretToken) {
  ...
  return extractPlaylistTrackOrder(metaResult.payload);
}
...
async function fetchAllSoundCloudTracksCollected({ accessToken, startPath, startQuery, normalizeRow }) {
...
async function soundCloudListLikedTracks({ accessToken, limit = 50, offset = 0 }) {
  void limit;
  void offset;
  return fetchAllSoundCloudTracksCollected({
    accessToken,
...
async function soundCloudListPlaylistTracks({
  ...
  const collected = await fetchAllSoundCloudTracksCollected({
    accessToken,
    startPath: `/playlists/${encodeURIComponent(playlistId)}/tracks`,
    startQuery: query,
    normalizeRow: normalizePlaylistTrackRow
  ...
  const idOrder = await fetchSoundCloudPlaylistTrackOrder({
    accessToken,
    playlistId,
    secretToken
  });
  const results = orderTracksByIdList(collected.results, idOrder);
  return {
    ok: true,
    results,
    nextOffset: null,
    collectionTotal: collected.collectionTotal
  };
}
...
module.exports = {
  ...
  orderTracksByIdList,
  fetchSoundCloudPlaylistTrackOrder,
