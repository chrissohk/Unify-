...
function normalizePlaylistTrackRowsWithPositions(rawItems, pageOffset) {
  const safeOffset = Math.max(Number(pageOffset) || 0, 0);
  const items = Array.isArray(rawItems) ? rawItems : [];
  const results = [];
  for (let i = 0; i < items.length; i++) {
    const row = normalizePlaylistTrackRow(items[i]);
    if (row) {
      row.playlistPosition = safeOffset + i;
      results.push(row);
    }
  }
  return results;
...
async function spotifyListSavedTracks({ sessions, persist, limit = 50, offset = 0 }) {
  ...
  const payload = await response.json();
  const rawItems = payload?.items || [];
  let results = normalizePlaylistTrackRowsWithPositions(rawItems, safeOffset);
...
function buildPlaylistTracksPageResult({ results, payload, pageOffset, limit }) {
  const safeLimit = Math.min(Math.max(Number(limit) || 50, 1), 50);
  const safeOffset = Math.max(Number(pageOffset) || 0, 0);
  const rawItems = payload?.items || [];
  ...
  return {
    ...
    tracksOlderOffset: computeTracksOlderOffset(safeOffset, safeLimit)
  };
}

async function fetchSpotifyPlaylistItemsPage({ sessions, persist, playlistId, limit, offset }) {
  const safeLimit = Math.min(Math.max(Number(limit) || 50, 1), 50);
  const safeOffset = Math.max(Number(offset) || 0, 0);
  const marketQ = spotifyMarketQuery(sessions);
  ...
  return buildPlaylistTracksPageResult({
    results: enrichResult.tracks,
    payload,
    pageOffset: safeOffset,
    limit: safeLimit
...
async function resolveSpotifyPlaylistTrackTotal({ sessions, persist, playlistId }) {
  const marketQ = spotifyMarketQuery(sessions);
  const playlistPath = `${SPOTIFY_API_BASE}/playlists/${encodeURIComponent(playlistId)}`;
  const metaUrl = `${playlistPath}?fields=items(total),tracks(total)${marketQ}`;
  const metaResult = await spotifyApiRequest({ sessions, persist, method: "GET", url: metaUrl });
  if (metaResult.ok && metaResult.response.ok) {
    const meta = await metaResult.response.json();
    const total = resolvePlaylistTrackCount(meta);
    if (typeof total === "number") return total;
  }
  const probe = await fetchSpotifyPlaylistItemsPage({
    sessions,
    persist,
    playlistId,
    limit: 1,
  ...
  return typeof probe.total === "number" ? probe.total : null;
}

/**
 * Tracks inside a playlist (offset pagination).
 * edge=newest fetches the last page for owned playlists (Spotify lists oldest-first).
 */
async function spotifyListPlaylistTracks({
  sessions,
  persist,
  playlistId,
  limit = 50,
  ...
  if (playlistId === SPOTIFY_LIKED_SONGS_ID) {
    const saved = await spotifyListSavedTracks({ sessions, persist, limit: safeLimit, offset: safeOffset });
    if (!saved.ok) {
      return saved;
    }
    return {
      ...saved,
      total: saved.collectionTotal,
      pageOffset: safeOffset,
      tracksOlderOffset: saved.nextOffset
    };
  }

  let fetchOffset = safeOffset;
  if (edge === "newest" && safeOffset === 0) {
    const total = await resolveSpotifyPlaylistTrackTotal({ sessions, persist, playlistId });
    if (typeof total === "number") {
      fetchOffset = computeTailPageOffset(total, safeLimit);
    }
  }

  return fetchSpotifyPlaylistItemsPage({
    sessions,
    persist,
    playlistId,
    limit: safeLimit,
